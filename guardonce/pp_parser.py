...
class DirectiveState:
    Empty, Hash, Pragma, Done, Ignore = range(5)
    ...
    while i <= len(src): # run off the end to process end-of-token
        ...
        if state == ParseState.Normal:
            ...
            if is_token_complete:
                ...
                token_start = -1
                if directive == DirectiveState.Hash:
                    if token == 'pragma':
            ...
            if c == '/':
                ...
                if directive == DirectiveState.Empty:
                    ...
                    directive_start = i
                elif directive != DirectiveState.Ignore:
                    raise ValueError('# in directive')
            elif is_name_char:
                if directive == DirectiveState.Empty:
                    directive = DirectiveState.Ignore
                elif directive == DirectiveState.Done:
                    raise ValueError('unrecognized value after pragma once')
                else:
                    if token_start <= 0:
                        token_start = i
            elif c == '\n':
                ...
                if directive == DirectiveState.Done:
                    raise ValueError('unrecognized value after pragma once')
